...
u8 DWord2BCD(u32 dwData, u8 len, u8* pdata)
{
    u32 index, count;
    ...
    {
        pdata[len-index-1] = (dwData/count) % 10;
        count = count*10;
        
        pdata[len-index-1] |= ((dwData/count) % 10) << 4;
        count = count*10;
    }
...
u8 util_string_number_counter(const u8 *pdata)
{
    u8 i=0;
    ...
    while(i<250)
    {
        if(pdata[i]<'0'||pdata[i]>'9') return i;
...
time_t util_bcd_to_sec(u8 *bcd)
{
    struct tm tm_t;

    ST_Time mt;

    mt.year = BCD2HEX(bcd[0]) + 2000;
