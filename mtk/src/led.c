...
typedef struct
{
    ...
    bool wifi_led_is_on;
	U16 wifi_led_state_time;
	LedState wifi_state;
	

}Led;

#define	WIFI_LED_PIN		GM_GPIO2

#define WIFI_LED_ON()     GM_GpioSetLevel(WIFI_LED_PIN,PINLEVEL_LOW);  
#define WIFI_LED_OFF()    GM_GpioSetLevel(WIFI_LED_PIN,PINLEVEL_HIGH);  
...
static void wifi_led_proc(void);
...
GM_ERRCODE led_create(void)
{
    ...
    s_led.wifi_led_is_on = false;
	s_led.wifi_led_state_time = 0;
	s_led.wifi_state = GM_LED_FLASH;

    GM_GpioInit(GM_GPIO2,PINDIRECTION_OUT,PINLEVEL_HIGH,PINPULLSEL_DISABLE);
...
static void wifi_led_proc(void)
{
    s_led.wifi_led_state_time += GM_LED_PROC_PERIOD_MS;

    //常亮或者长灭
	if (s_led.wifi_state != GM_LED_FLASH)
	{
		 s_led.wifi_led_is_on = false;
		 WIFI_LED_OFF();
		 s_led.wifi_led_state_time = 0;
	...
	{
		if (s_led.wifi_led_state_time >= GM_LED_QUICK_FLASH_PERIOD_MS*5)
        {
			s_led.wifi_led_is_on = !s_led.wifi_led_is_on;
            GM_GpioSetLevel(WIFI_LED_PIN,(s_led.wifi_led_is_on == true?PINLEVEL_HIGH:PINLEVEL_LOW));
			s_led.wifi_led_state_time = 0;
        } 
	}
}
